...
function App() {
  ...
  const getUsers = async (since = "") => {
    try {
      const response = await axios.get(
        `https://backk-e4ou.onrender.com/api/users?since=${since}`
      );
  ...
  const detailUser = async (username) => {
    try {
      const response = await axios.get(
        `https://backk-e4ou.onrender.com/api/users/${username}`
      );
  ...
  const getRepos = async (username) => {
    try {
      detailUser(username);
      const response = await axios.get(
        `https://backk-e4ou.onrender.com/api/user/repos?username=${username}`
      );
  ...
  const handleDetailsClick = async (user) => {
    ...
    setShowDetails((prevShowDetails) => ({
      ...prevShowDetails,
      [user.id]: !prevShowDetails[user.id],
    }));
  };
